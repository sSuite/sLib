...
import com.github.ssuite.slib.event.ConfirmationAcceptEvent;
import com.github.ssuite.slib.event.ConfirmationRejectEvent;
import com.github.ssuite.slib.event.ConfirmationTimedOutEvent;
...
public class ConfirmationService {
	...
	public ConfirmationService(Plugin plugin) {
		this.plugin = plugin;
		random = new Random();
	...
	public void acceptRequest(String token) {
		if (hasToken(token)) {
			plugin.getServer().getPluginManager().callEvent(new ConfirmationAcceptEvent(getPlayer(token), token));
			removeToken(token);
		}
	}
	...
	public void rejectRequest(String token) {
		if (hasToken(token)) {
			plugin.getServer().getPluginManager().callEvent(new ConfirmationRejectEvent(getPlayer(token), token));
			removeToken(token);
		}
	}
}